...
alter table @OWNER_SCHEMA.city add constraint FK_City_0 foreign key (country_id) references @OWNER_SCHEMA.country(Id);
...
alter table @OWNER_SCHEMA.film_actor add constraint FK_FilmActor_0 foreign key (film_id) references @OWNER_SCHEMA.film(Id);


alter table @OWNER_SCHEMA.film_category add constraint FK_FilmCategory_0 foreign key (film_id) references @OWNER_SCHEMA.film(Id);
alter table @OWNER_SCHEMA.film_category add constraint FK_FilmCategory_1 foreign key (category_id) references @OWNER_SCHEMA.category(Id);


alter table @OWNER_SCHEMA.film add constraint FK_Film_0 foreign key (language_id) references @OWNER_SCHEMA.language(Id);
alter table @OWNER_SCHEMA.film add constraint FK_Film_1 foreign key (original_language_id) references @OWNER_SCHEMA.language(Id);
