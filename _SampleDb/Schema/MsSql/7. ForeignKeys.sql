alter table dbo.city add constraint FK_City_0 foreign key (country_id) references dbo.country(Id);
...
alter table dbo.film_actor add constraint FK_FilmActor_0 foreign key (film_id) references dbo.film(Id);


alter table dbo.film_category add constraint FK_FilmCategory_0 foreign key (film_id) references dbo.film(Id);
alter table dbo.film_category add constraint FK_FilmCategory_1 foreign key (category_id) references dbo.category(Id);


alter table dbo.film add constraint FK_Film_0 foreign key (language_id) references dbo.language(Id);
alter table dbo.film add constraint FK_Film_1 foreign key (original_language_id) references dbo.language(Id);
